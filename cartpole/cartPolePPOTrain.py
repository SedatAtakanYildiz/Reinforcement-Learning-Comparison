...
success_rate = np.sum(np.array(rewards) >= 195) / episodes  # Başarı eşiği: 195
...
print(f"Başari Orani: {success_rate * 100:.2f}%")

# Kayıt klasörü oluştur
os.makedirs("results", exist_ok=True)

# CSV kayıt
with open("results/ppo_cartpole_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
# Grafik
plt.plot(rewards)
plt.title("PPO - CartPole Learning Curve (500 Episode)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.grid()
plt.savefig("results/ppo_cartpole_learning_curve.png")
plt.close()