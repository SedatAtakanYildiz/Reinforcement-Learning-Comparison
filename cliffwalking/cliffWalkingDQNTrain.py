...
model.save("dqn_cliffwalking_model")
...
success_rate = np.sum(np.array(rewards) >= -100) / episodes  # örnek başarı kriteri
...
# CSV kaydı
with open("results/dqn_cliffwalking_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
# Grafik
plt.plot(rewards)
plt.title("DQN - CliffWalking Learning Curve (500 Episode)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.grid()
plt.savefig("results/dqn_cliffwalking_learning_curve.png")
plt.close()