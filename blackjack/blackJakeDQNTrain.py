...
# Ortamı oluştur
env = FlattenObservation(gym.make("Blackjack-v1", sab=True))
...
success_rate = np.sum(np.array(rewards) > 0) / episodes  # Pozitif ödül = başarı
...
# CSV kaydı
with open("results/dqn_blackjack_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
# Grafik
plt.plot(rewards)
plt.title("DQN - Blackjack Learning Curve (500 Episode)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.grid()
plt.savefig("results/dqn_blackjack_learning_curve.png")
plt.close()