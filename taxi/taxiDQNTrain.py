...
if device_type == "cuda":
    print("Kullanılan cihaz:", torch.cuda.get_device_name(0))
...
success_rate = np.sum(np.array(rewards) >= 7) / episodes  # Başarı eşiği: 7
...
print(f"Başarı Oranı (>=7): {success_rate * 100:.2f}%")

# Kayıt klasörü
os.makedirs("results", exist_ok=True)

# CSV kaydı
with open("results/dqn_taxi_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
# Grafik
plt.plot(rewards)
plt.title("DQN - Taxi Learning Curve (500 Episode)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.grid()
plt.savefig("results/dqn_taxi_learning_curve.png")
plt.close()