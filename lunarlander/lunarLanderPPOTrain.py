...
success_rate = np.sum(np.array(rewards) >= 200) / episodes  # Başarı kriteri: ödül >= 200
...
print(f"Başarı Oranı (>=200): {success_rate * 100:.2f}%")

# Kayıt klasörü
os.makedirs("results", exist_ok=True)

# CSV kaydı
with open("results/ppo_lunarlander_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["env_id", "mean_reward", "std_reward", "training_time", "success_rate", "max_reward", "min_reward"])
    writer.writerow(["LunarLander-v2", mean_reward, std_reward, training_time, success_rate, max_reward, min_reward])

# Grafik
plt.plot(rewards)
plt.title("PPO - LunarLander Learning Curve (500 Episode)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.grid()
plt.savefig("results/ppo_lunarlander_learning_curve.png")
plt.close()