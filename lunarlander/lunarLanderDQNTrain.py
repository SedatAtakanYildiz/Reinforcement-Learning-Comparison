...
success_rate = np.sum(np.array(rewards) >= 200) / episodes  # başarı kriteri: 200 puan
...
# CSV kaydı
with open("results/dqn_lunarlander_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["env_id", "mean_reward", "std_reward", "training_time", "success_rate", "max_reward", "min_reward"])
    writer.writerow(["LunarLander-v3", mean_reward, std_reward, training_time, success_rate, max_reward, min_reward])

# Grafik
plt.plot(rewards)
plt.title("DQN - LunarLander Learning Curve (500 Episode)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.grid()
plt.savefig("results/dqn_lunarlander_learning_curve.png")
plt.close()